...
@section('content')
	...
	<div class="container-fluid">
		...
		<form action="{{ $template['action'] }}" method="POST" id="formEdit" name="formEdit" data-key="{{$province->id ?? '0'}}">
			@csrf
					...
					<div class="card card-primary">
						...
						<div class="card-body">
							...
							<div class="form-group">
								...
								<input type="text" id="code" name="code" value="{{ old('code', $province->code ?? '') }}" class="form-control @error('code') is-invalid @enderror">
								@error('code')
							...
							<div class="form-group">
								...
								<input type="text" id="name" name="name" value="{{ old('name', $province->name ?? '') }}" class="form-control @error('name') is-invalid @enderror" >
								@error('name')
							...
							<div class="form-group">
								...
								<select id="is_active" name="is_active" class="form-control custom-select  @error('is_active') is-invalid @enderror">
									...
									@foreach ($ddnActiveStatus as $keyActiveStatus => $valActiveStatus)
										<option value="{{$keyActiveStatus}}" {{ ($keyActiveStatus == old('is_active', $province->active_status ?? '')) ? "selected" : "" }}>{{$valActiveStatus}}</option>
									@endforeach
								</select>
								@error('is_active')
